...
func (db *Database) ItemGetPrefetch(lim int) ([]feed.Item, error) {
	const qid query.ID = query.ItemGetPrefetch
	var (
		err  error
		stmt *sql.Stmt
	)
...
EXEC_QUERY:
	...
	var items = make([]feed.Item, 0, lim)
...
func (db *Database) ItemPrefetchSet(i *feed.Item, description string) error {
	const qid query.ID = query.ItemPrefetchSet
	var (
		err    error
		msg    string
		stmt   *sql.Stmt
...
EXEC_QUERY:
	if _, err = stmt.Exec(description, i.ID); err != nil {
		if worthARetry(err) {
			waitForRetry()
			goto EXEC_QUERY
		} else {
...
// ClusterCreate creates a new Cluster in the database and returns it.
func (db *Database) ClusterCreate(name, description string) (*cluster.Cluster, error) {
	const qid query.ID = query.ClusterCreate
	var (
		err    error
		msg    string
		stmt   *sql.Stmt
...
EXEC_QUERY:
	if res, err = stmt.Exec(name, description); err != nil {
		if worthARetry(err) {
			waitForRetry()
			goto EXEC_QUERY
		} else {
			err = fmt.Errorf("Cannot add cluster %s (%s) to database: %s",
				name,
				description,
				err.Error())
			db.log.Printf("[ERROR] %s\n", err.Error())
		...
		if clusterID, err = res.LastInsertId(); err != nil {
			db.log.Printf("[ERROR] Cannot get ID of new Cluster %q: %s\n",
				name,
				err.Error())
			return nil, err
		}
...
} // func (db *Database) ClusterCreate(c *cluster.Cluster) error

// ClusterDelete removes a Cluster from the database.
func (db *Database) ClusterDelete(id int64) error {
	const qid query.ID = query.ClusterDelete
	var (
		err    error
		msg    string
		stmt   *sql.Stmt
...
EXEC_QUERY:
	if _, err = stmt.Exec(id); err != nil {
		if worthARetry(err) {
			...
			err = fmt.Errorf("cannot delete Cluster %d: %s",
				id,
				err.Error())
			db.log.Printf("[ERROR] %s\n", err.Error())
			return err
...
// ClusterGetByID looks up a Cluster by its ID.
func (db *Database) ClusterGetByID(id int64) (*cluster.Cluster, error) {
	const qid query.ID = query.ClusterGetByID
	var (
		err  error
		stmt *sql.Stmt
	)
...
EXEC_QUERY:
	...
	if rows.Next() {
		...
		if err = rows.Scan(&c.Name, &c.Description, &stamp); err != nil {
			db.log.Printf("[ERROR] Cannot scan row: %s\n",
				err.Error())
			return nil, err
		}
...
EXEC_QUERY:
	...
	defer rows.Close() // nolint: errcheck,gosec
	var results = make([]cluster.Cluster, 0, 8)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&c.ID, &c.Name, &c.Description, &stamp); err != nil {
			db.log.Printf("[ERROR] Cannot scan row: %s\n",
				err.Error())
			return nil, err
		}
...
EXEC_QUERY:
	...
	defer rows.Close() // nolint: errcheck,gosec
	var results = make([]feed.Item, 0, 8)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&i.ID, &i.FeedID, &i.URL, &i.Title, &desc, &stamp, &i.Read, &rating); err != nil {
			db.log.Printf("[ERROR] Cannot scan row: %s\n",
				err.Error())
			return nil, err
		}
...
// ClusterGetAll fetches all Clusters from the Database.
// If fetchItems is true, it also fetches the Items for each Cluster.
func (db *Database) ClusterGetAll(fetchItems bool) ([]cluster.Cluster, error) {
	const qid query.ID = query.ClusterGetAll
	var (
		err  error
		stmt *sql.Stmt
	)
...
EXEC_QUERY:
	if rows, err = stmt.Query(); err != nil {
		...
		db.log.Printf("[ERROR] Cannot load Clusters: %s\n",
			err.Error())
		return nil, err
	}
	...
	for rows.Next() {
		...
		if fetchItems {
			if c.Items, err = db.ClusterGetItems(c.ID); err != nil {
				db.log.Printf("[ERROR] Cannot fetch Items for Cluster %q (%d): %s\n",
					c.Name,
					c.ID,
					err.Error())
				return nil, err
...
// ClusterNameUpdate renames a Cluster.
func (db *Database) ClusterNameUpdate(c *cluster.Cluster, name string) error {
	const qid query.ID = query.ClusterNameUpdate
	var (
		err    error
		msg    string
		stmt   *sql.Stmt
...
EXEC_QUERY:
	if _, err = stmt.Exec(name, c.ID); err != nil {
		if worthARetry(err) {
			waitForRetry()
			goto EXEC_QUERY
		} else {
			err = fmt.Errorf("Cannot rename cluster %s (%d) to %s: %s",
				c.Name,
				c.ID,
				name,
				err.Error())
...
// ClusterLinkAdd adds an Item to an existing Cluster.
func (db *Database) ClusterLinkAdd(clusterID, itemID int64) error {
	const qid query.ID = query.ClusterLinkAdd
	var (
		err    error
		msg    string
		stmt   *sql.Stmt
...
EXEC_QUERY:
	if _, err = stmt.Exec(clusterID, itemID); err != nil {
		if worthARetry(err) {
			waitForRetry()
			goto EXEC_QUERY
		} else {
			err = fmt.Errorf("Cannot add Item #%d to Cluster #%d: %s",
				itemID,
				clusterID,
				err.Error())
			db.log.Printf("[ERROR] %s\n", err.Error())
...
// ClusterLinkDel removes the given Item from the given Cluster.
func (db *Database) ClusterLinkDel(clusterID, itemID int64) error {
	const qid query.ID = query.ClusterLinkDel
	var (
		err    error
		msg    string
		stmt   *sql.Stmt
...
EXEC_QUERY:
	if _, err = stmt.Exec(clusterID, itemID); err != nil {
		if worthARetry(err) {
			...
			err = fmt.Errorf("Cannot remove Item #%d from Cluster #%d: %s",
				itemID,
				clusterID,
				err.Error())
			db.log.Printf("[ERROR] %s\n", err.Error())
